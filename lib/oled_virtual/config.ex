defmodule OLEDVirtual.Config do
  ...
  def validate(%{on_display: [mod, func]}, :on_display) when is_atom(mod) and is_atom(func) do
    case function_exported?(mod, func, 2) do
      ...
      false ->
        {:error,
         "optional config value 'on_display' must be of the format '[module, function]' where function has an arity of 2"}
    end
  end
  ...
  def validate(%{on_buffer_update: [mod, func]}, :on_buffer_update)
    ...
    case function_exported?(mod, func, 2) do
      ...
      false ->
        {:error,
         "optional config value 'on_buffer_update' must be of the format '[module, function]' where function has an arity of 2"}
    end
  end
